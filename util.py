...
def getTurtleAndScreen(title, width, height, moveWorld=True):
    ...
    canvas = ResizingCanvas(root, desired_width=width, desired_height=height, width=width, height=height,
    ...
    if moveWorld:
        # Need the -1s in the code below because it appears that if we use 0 as the boundary, we don't
        # see things drawn at 0, so we start at -1 instead.
    ...
    turtle = RawTurtle(screen, visible=True)
